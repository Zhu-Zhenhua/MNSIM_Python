...
class awnas_TrainTestInterface(object):
    def __init__(self, network_module, dataset_module, SimConfig_path,net, _param_list = None, device = None, extra_define = None):
        ...
        self.net=net
        #torch.save("weight.pth", self.net.state_dict())
        # a = self.net.state_dict()
        # print(a.keys())
        # self.net.saved_dict()
        if _param_list is not None:
            #保存下权重
            # print(f'load weights from {_param_list}')
...
class awnas_NetworkGraph(nn.Module):
    ...
    def load_change_weights(self, _param_list):
        ...
        for i in range(len(_param_list)):
            # print(_param_list[i].keys())
            if self.layer_list[i].layer_config['type']=='fc' or self.layer_list[i].layer_config['type']=='conv':
                # print(_param_list[i].items())
                state_dict[f'layer_list.{i}.bit_scale_list']=torch.FloatTensor([[9, 1],[9, 1],[9, 1]])
                if 'bias' in _param_list[i].keys():
                    _param_list[i].pop("bias")
                if 'weight' in _param_list[i].keys():
                    state_dict[f'layer_list.{i}.layer_list.{i}.weight']=_param_list[i]["weight"]
            # if self.layer_con
            if self.layer_list[i].layer_config['type']=='bn':
                ...
                state_dict[f'layer_list.{i}.layer.running_mean']= _param_list[i]["running_mean"]
                state_dict[f'layer_list.{i}.layer.running_var']=_param_list[i]["running_var"]
            state_dict[f'layer_list.{i}.last_value']=torch.FloatTensor([1])
        ...
        for key in state_dict.keys():
            ...
            # print("key:{},tmp_key:{}".format(key,tmp_key))
